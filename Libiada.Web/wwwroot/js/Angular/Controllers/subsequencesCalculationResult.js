function SubsequencesCalculationResultController() {
    ...
    function subsequencesCalculationResult($scope, $http, $sce) {
        ...
        function fillBarPlotData() {
            ...
            let min = Math.min(...$scope.points.map(p => p.map(pi => pi.characteristics[characteristicIndex])));
            let max = Math.max(...$scope.points.map(p => p.map(pi => pi.characteristics[characteristicIndex])));
            let range = Math.abs(max - min);
            let maxNameLength = Math.max(...$scope.points.map(p => p.map(pi => pi.name.length)));
            ...
            $scope.chartData = $scope.points.map(p => ({
                hoverinfo: "text+x+y",
                x: [p.map(pi => pi.name)],
                y: [p.map(pi => pi.characteristics[characteristicIndex])],
                marker: { color: $scope.legend[p[0].legendIndex].color },
                type: "scatter",
                customdata: { legendId: p[0].legendId },
                name: p.map(pi => pi.name),
                visible: $scope.legend[p[0].legendIndex].visible ? "true" : "legendonly"
            }));
        }
