function CharacteristicsTableExcelExporterController() {
    const ctrl = this;
    ctrl.exportToExcel = async function exportToExcel() {
        ...
        if (ctrl.sequenceGroups)
            columns.push({ header: "Sequences group", key: "sequenceGroup", width: 10, style: { font: font, border: border } });
        ...
        for (let i = 0; i < ctrl.characteristics.length; i++) {
            let row = { id: i + 1, name: ctrl.characteristics[i].ResearchObjectName };
            if (ctrl.sequenceGroups)
                row.sequenceGroup = ctrl.characteristics[i].SequenceGroupId;
...
export {};
//# sourceMappingURL=characteristicsTableExcelExporter.js.map