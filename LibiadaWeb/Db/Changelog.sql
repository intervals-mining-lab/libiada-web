...
-- Добавлено ограничение уникальности для занчений и формы записи элементов.
-- Исправлена функция проверки алфавита.
-- Добавлено ограничение уникальности литературных цепочек.

ALTER TABLE element ADD CONSTRAINT uk_element_value_notation UNIQUE (value, notation_id);
...
ALTER TABLE literature_chain ADD CONSTRAINT uk_literature_chain UNIQUE (notation_id, matter_id, piece_type_id, piece_position, language_id, translator_id);

-- 07.05.2014

-- Переименован ключ.

ALTER TABLE binary_characteristic DROP CONSTRAINT fk_characteristic_type;

ALTER TABLE binary_characteristic ADD CONSTRAINT fk_binary_characteristic_characteristic_type FOREIGN KEY (characteristic_type_id) REFERENCES characteristic_type (id) MATCH SIMPLE ON UPDATE CASCADE ON DELETE NO ACTION;

-- 12.05.2014

-- Переименовал класс калькулятора мощности алфавита.

UPDATE characteristic_type SET class_name = 'AlphabetCardinality' WHERE class_name = 'AlphabetPower';

-- 19.06.2014

-- Исправил тип фрагмента у рибосомальных РНК.
